...
public class AssetDamageStreamProcessor extends AbstractStreamProcessor {
    ...
    private record AttachmentUploadContext(EvtDamageAttachment model, String base64Content) {}
    ...
    protected List<Model> buildModels(String messageId, String eventType, Map<String, Object> payload) {
        ...
        // Attachment models (id_asset_damage propagated in saveModels)
        pendingAttachments = new ArrayList<>();
        List<Map<String, Object>> attachments =
                (List<Map<String, Object>>) payload.get("assetDamageAttachments");
        if (attachments != null) {
            for (Map<String, Object> att : attachments) {
                ...
                a.set("edit_user_id",    getInteger(att, "editUserId"));
                a.set("asset_damage_id", getLong(att, "assetDamageId"));
                a.set("create_time",     parseTimestamp(att, "createTime"));
                a.set("edit_time",       parseTimestamp(att, "editTime"));
    ...
    // --- Save override: propagate parent's generated ID to labels and attachments ---
    ...
    protected void saveModels(List<Model> models, LookupResult lookup) {
        ...
        for (int i = 1; i < models.size(); i++) {
            ...
            child.set("id_asset_damage", actualId);

            if (child instanceof EvtDamageAttachment attachment) {
    ...
    private Long uploadAttachment(EvtDamageAttachment attachment, Object parentId, String base64Content) {
        String fileName = (String) attachment.get("filename");
        try {
            byte[] bytes = Base64.getDecoder().decode(base64Content);
            return berlinkAttachmentService.upload(fileName, bytes, "ASSET_DAMAGE", parentId);
        } catch (Exception e) {
            log.warn("Failed to upload attachment fileName={}, id_asset_damage={}: {}",
    ...
    protected int deleteByMessageId(String messageId) {
        // Cascade: delete associated attachments and labels before the main record
        EvtAssetDamage existing = EvtAssetDamage.findByMessageId(messageId);
        if (existing != null) {
            long idAssetDamage = ((Number) existing.getId()).longValue();
            List<Long> idDocuments = EvtDamageAttachment.findIdDocumentsByAssetDamageId(idAssetDamage);
            for (Long idDocument : idDocuments) {
                berlinkAttachmentService.delete(idDocument);
            }
            EvtDamageAttachment.deleteByAssetDamageId(idAssetDamage);
